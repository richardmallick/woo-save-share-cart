...
<table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
	...
	foreach ( $cart as $cart_item_key => $cart_item ) {
		...
		if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
			...
			<td class="product-price" data-title="<?php esc_attr_e( 'Price', 'wcssc' ); ?>">
				...
				echo wc_price($cart_item['line_total'] / $product_quantity);
			...
			<td class="product-subtotal" data-title="<?php esc_attr_e( 'Total', 'wcssc' ); ?>">
				...
				echo wc_price($cart_item['line_total']); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				...
				$total_tax = $total_tax + $cart_item['line_tax'];
				//$total_price = $total_price + ( $product_price * $product_quantity );
				?>
			</td>
	...
	<tr>
		...
		<?php
			...
			echo sprintf( __( 'Total Price: %s', 'wcssc' ), wc_price( $total_price + $total_tax ) );
		?>
		</td>
	</tr>
	</tbody>
